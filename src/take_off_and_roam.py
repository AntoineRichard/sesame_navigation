...
class SimpleTakeOffAndStay:
    def __init__(self):
        self.take_off_altitude = rospy.get_param("~take_off_altitude", 0.8)
        self.rate = rospy.get_param("~rate", 30)
        self.timeout = rospy.get_param("~timeout", 10)
        self.uav_name = rospy.get_param("~uav_name", "/uav_b")
        ...
        self.local_pose_sub = rospy.Subscriber(
            "/uav_b/mavros/local_position/pose", PoseStamped, self.localPoseCallback
        )
        self.state_sub = rospy.Subscriber(
            "/uav_b/mavros/state", State, self.stateCallback
        )
        self.take_off_srv = rospy.Service(
            "/uav_b/take_off", Trigger, self.takeOffRequest
        )
        self.land_srv = rospy.Service("/uav_b/land", Trigger, self.landRequest)

        self.pose_pub = rospy.Publisher(
            "/uav_b/mavros/setpoint_position/local", PoseStamped, queue_size=1
        )
        ...
        while (not rospy.is_shutdown()) and (self.landing_requested == False):
            ...
            if self.state.armed == False:
                ...
                if delta.to_sec() > self.timeout:
                    ...
                    rospy.loginfo("Current state: {}".format(self.state.mode))
                    break
            x = self.stay_coordinates[0] + radius * np.cos(
                2
                * np.pi
                * rotations_per_second
                * (rospy.Time.now() - starting_time).to_sec()
            )
            y = self.stay_coordinates[1] + radius * np.sin(
                2
                * np.pi
                * rotations_per_second
                * (rospy.Time.now() - starting_time).to_sec()
